"""
FactcheckOrchestrator — мультиагентный фактчек с инструментами верификации.
Version: 1.1.0
Description: N чекеров с web_search/web_read/code_execute → арбитр → итоговый отчёт.
"""
import logging
import re
...
## 5. Вывод
Помни: не размечай лишнего; если источники нельзя ранжировать — ошибкой НЕ считаем.
- Если чекер привёл данные первоисточника с рабочим URL (цифры, название) —
  прими их как корректное значение, даже если второй чекер молчал.
- Ошибки в официальных названиях и в направлении динамики («выросла/снизилась»,
  ...
  не снимай их только за то, что второй чекер их не заметил.
"""
...
class FactcheckOrchestrator:
    def __init__(self, client, settings, registry: ToolRegistry):
        ...
        self.registry = registry  # ← ДОБАВЛЕНО: реестр инструментов
        self.prompt_loader = PromptLoader()
        self.usage = UsageTracker()
        self.skill_context = load_skill("fact-checking")
        self.settings.output_dir.mkdir(parents=True, exist_ok=True)
        
        # Получаем инструменты для fact-checking навыка
    ...
    def _run_checker(self, idx: int, model: str, docs) -> str:
        ...
        try:
            report = _make_agent().run(msg, max_iterations=self.settings.checker_max_iterations)
            ...
            try:
                report = _make_agent().run(
                    ...
                    max_iterations=self.settings.checker_max_iterations,
                )
            except Exception as e2:                     # 503 и пр. — не роняем батч
                logger.error(f"❌ Чекер {idx} ({model}) недоступен: {e2}")
                return f"❌ Чекер недоступен (API error): {e2}. Проверка не выполнена."
        if self._report_broken(report):
            ...
            try:
                report2 = _make_agent().run(msg + nudge,
                                            max_iterations=self.settings.checker_max_iterations)
                if not self._report_broken(report2):
                    report = report2
            except Exception as e:
    ...
    def _arbitrate(self, reports: List[Dict]) -> str:
        ...
        task = (f"Ниже — отчёты {len(reports)} чекеров по ОДНИМ материалам. "
